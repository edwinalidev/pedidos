...
function getPedidoDetalle($pdo, $idPedido)
{
    ...
    foreach ($dataPedido as $row) {
        $detalleStmt = $pdo->prepare("SELECT d.idItem,d.Detalle,d.Precio,d.Cantidad,i.Unidad FROM detalles d inner join items i on d.idItem=i.idItem WHERE d.idPedido = :idPedido order by idDetalle ");
